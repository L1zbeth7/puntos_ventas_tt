...
class ConsultaVentasController extends ChangeNotifier {
  ...
  List<VentaModelV> get ingresos => _ventas;
  List<VentaModelV> get ingresosBuscar => _ventasBuscar;
  ...
  Future<void> getReporte({bool buscar = false}) async {
    ...
    for (int i = 0; i < categoriasReporte.length; i++) {
      ...
      suma += categoriasReporte[i].totalVenta;
    }
  ...
  void buscarIngreso() {
    ...
    for (var element in _ventas) {
      ...
      var proveedor = element.cliente.nombre
          .toLowerCase()
      ...
      if (fecha ||
          ...
          (totalVenta == null
              ? false
              : int.parse(buscar.text) == element.totalVenta)) {
